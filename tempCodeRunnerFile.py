...
def create_embeddings(text_list):
    r = request.post("http://localhost:11434/api/embed", json={
        "model": "bge-m3",
        "input": text_list
    })
...
def find_simmilar_chunks(df, query, top_k=5):
    """
...
def inference(prompt):
    ...
    r = request.post("http://localhost:11434/api/generate", json={
        "model": "llama3.2:3b",
        "prompt": prompt,
...
if __name__ == "__main__":
    ...
    while True:
        ...
        prompt = f'''I am teaching web development in my Sigma web development course. Here are video subtitle chunks containing video title, video number, start time, end time, and the text at that time:
